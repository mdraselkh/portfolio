...
const Portfolio = () => {
  ...
  useEffect(()=>{
    ...
    if(selecttab==='web'){
      const filtereddata= portfoliodata.filter(item=> item.category==='Web')
      setportfolios(filtereddata);
    }

    if(selecttab==='ml'){
      const filtereddata= portfoliodata.filter(item=> item.category === 'ML')
      setportfolios(filtereddata);
    }
    ...
    <div name="portfolio" className="bg-gray-900 mt-[-1px] py-4">
      ...
      <div className="flex items-center justify-between py-6 mx-10 lg:mx-48 md:mx-16">
        ...
        <div className="flex flex-col items-center justify-between gap-2 md:flex-row md:gap-4">
          <button onClick={()=> setselectedtab('all')} className="px-4 font-semibold text-gray-300 duration-500 border-2 border-orange-500 rounded cursor-pointer hover:bg-gradient-to-l from-orange-300 to-orange-500 hover:text-gray-800 hover:border-none md:px-6 md:py-2 md:items-start">
            All
          </button>
          <button onClick={()=> setselectedtab('web')} className="px-4 font-semibold text-gray-300 duration-500 border-2 border-orange-500 rounded cursor-pointer hover:bg-gradient-to-l from-orange-300 to-orange-500 hover:text-gray-800 hover:border-none md:px-6 md:py-2 md:items-start">
            Web
          </button>
          <button onClick={()=> setselectedtab('ml')} className="px-4 font-semibold text-gray-300 duration-500 border-2 border-orange-500 rounded cursor-pointer hover:bg-gradient-to-l from-orange-300 to-orange-500 hover:text-gray-800 hover:border-none md:px-6 md:py-2 md:items-start">
            ML
          </button>
        </div>
      </div>
      <div className="grid grid-cols-1 gap-5 py-6 mx-10 lg:grid-cols-3 md:gap-8 lg:mx-48 md:mx-16 md:grid-cols-2">
        {portfolios.slice(0, nextitems)?.map((data) => (
          <div
            key={data.id}
            className={"flex flex-col items-center max-w-full rounded-lg group" + 
            " " + data.style}
            data-aos="fade-zoom-in" data-aos-duration='1200' data-aos-delay='50'
          >
            ...
            <div className="flex items-center justify-between w-full p-4 mb-0 bg-gray-600 rounded-b">
               <a href={data.siteurl} className="px-3 py-1 font-semibold text-white border-2 border-orange-500 rounded cursor-pointer hover:bg-gradient-to-t from-orange-300 to-orange-500 hover:text-gray-800 hover:border-none">Demo</a>
               <a href={data.githuburl} className="px-3 py-1 font-semibold text-white border-2 border-orange-500 rounded cursor-pointer hover:bg-gradient-to-t from-orange-300 to-orange-500 hover:text-gray-800 hover:border-none">Code</a>
            </div>
          </div>
        ))}
      </div>
